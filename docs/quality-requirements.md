...
- **Scenario:** When the system loads the game scene under standard production-like conditions, the `GridGenerator` component shall spawn the initial grid of blocks within 3 seconds.
...
- **Scenario:** When the system instantiates a block GameObject without required block type data under the standard Unity runtime environment, the `BlockBehaviour` component shall log an error and prevent an application crash within 1 frame.
...
- **Scenario:** When the system loads the game scene under standard conditions, the `InGameMenuManager` artifact shall ensure the game state is fully unpaused (`Time.timeScale = 1`) within 1 frame.
